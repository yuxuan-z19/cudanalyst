...
SimulationData move_simulation_data_to_device(Inputs in, int mype,
    ...
    // Length = length_index_grid NuclideGridPoint * nuclide_grid;    // Length
    // = length_nuclide_grid
    //
    ...
    gpuErrchk(cudaMemcpy(GSD.unionized_energy_array, SD.unionized_energy_array,
    ...
    gpuErrchk(cudaMemcpy(GSD.nuclide_grid, SD.nuclide_grid, sz,
                         cudaMemcpyHostToDevice));
    total_sz += sz;

    // Allocate verification array on device. This structure is not needed on
    // CPU, so we don't have to copy anything over.
    ...
    if (mype == 0)
        printf(
            "GPU Intialization complete. Allocated %.0lf MB of data on GPU.\n",
...
SimulationData grid_init_do_not_profile(Inputs in, int mype) {
    ...
    for (int i = 0; i < in.n_isotopes; i++)
        qsort(&SD.nuclide_grid[i * in.n_gridpoints], in.n_gridpoints,
    ...
    for( int i = 0; i < in.n_isotopes; i++ )
    {
        ...
        for( int j = 0; j < in.n_gridpoints; j++ )
            printf("E%d = %lf\n", j, SD.nuclide_grid[i * in.n_gridpoints +
    j].energy);
    }
    */
    ...
    if (in.grid_type == UNIONIZED) {
        ...
        qsort(SD.unionized_energy_array, SD.length_unionized_energy_array,
        ...
        for (long e = 0; e < SD.length_unionized_energy_array; e++) {
            ...
            for (long i = 0; i < in.n_isotopes; i++) {
                if (unionized_energy < energy_high[i])
                    ...
                    SD.index_grid[e * in.n_isotopes + i] = idx_low[i];
                    energy_high[i] =
                        SD.nuclide_grid[i * in.n_gridpoints + idx_low[i] + 1]
                            .energy;
                }
            }
        }
    ...
    if (in.grid_type == HASH) {
        ...
        for (long e = 0; e < in.hash_bins; e++) {
            ...
            for (long i = 0; i < in.n_isotopes; i++) {
                SD.index_grid[e * in.n_isotopes + i] =
                    grid_search_nuclide(in.n_gridpoints, energy,
                                        SD.nuclide_grid + i * in.n_gridpoints,
                                        0, in.n_gridpoints - 1);
            }
        }
    }
    ...
    // Intialize the flattened 2D grid of nuclide concentration data. The grid
    // holds a list of nuclide concentrations for each of the 12 material types.
    // The grid is allocated as a full square grid, even though not all
    // materials have the same number of nuclides.
