...
unsigned long long run_event_based_simulation_baseline(Inputs in,
                                                       SimulationData GSD,
                                                       int mype) {
    ////////////////////////////////////////////////////////////////////////////////
    ...
    if (grid_type == NUCLIDE) {
        ...
        if (idx == n_gridpoints - 1)
            ...
            low = &nuclide_grids[nuc * n_gridpoints + idx];
               ...
               UNIONIZED)  // Unionized Energy Grid - we already know the index,
...
__global__ void xs_lookup_kernel_optimization_3(Inputs in, SimulationData GSD,
    ...
    if (((is_fuel == 1) && (mat == 0)) || ((is_fuel == 0) && (mat != 0))) {
        ...
        calculate_macro_xs(
            ...
            in.n_gridpoints,  // Number of gridpoints per isotope in simulation
            GSD.num_nucs,     // 1-D array with number of nuclides per material
            GSD.concs,        // Flattened 2-D array with concentration of each
                              // nuclide in each material
            GSD.unionized_energy_array,  // 1-D Unionized energy array
            GSD.index_grid,  // Flattened 2-D grid holding indices into nuclide
            ...
            in.hash_bins,     // Number of hash bins used (if using hash lookup
                              // type)
            GSD.max_num_nucs  // Maximum number of nuclides present in any
                              // material
        );
