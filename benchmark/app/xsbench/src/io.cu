...
// If running in MPI, reduce timing statistics and calculate average
    ...
    if (mype == 0) {
        ...
        // Print the results
        printf(
            "NOTE: Timings are estimated -- use nvprof/nsys/iprof/rocprof for "
            "formal analysis\n");
#ifdef MPI
        printf("MPI ranks:   %d\n", nprocs);
#endif
    ...
    if (mype == 0) {
        if (is_invalid_result)
            printf(
                "Verification checksum: %llu (WARNING - INVALID CHECKSUM!)\n",
                vhash);
        else
...
void print_CLI_error(void) {
    ...
    printf(
        "  -s <size>                Size of H-M Benchmark to run (small, "
        "large, XL, XXL)\n");
    printf(
        "  -g <gridpoints>          Number of gridpoints per nuclide "
        "(overrides -s defaults)\n");
    printf(
        "  -G <grid type>           Grid search type (unionized, nuclide, "
        "hash). Defaults to unionized.\n");
    printf("  -p <particles>           Number of particle histories\n");
    printf(
        "  -l <lookups>             History Based: Number of Cross-section "
        "(XS) lookups per particle. Event Based: Total number of XS "
        "lookups.\n");
    printf(
        "  -h <hash bins>           Number of hash bins (only relevant when "
        "used with \"-G hash\")\n");
    printf(
        "  -b <binary mode>         Read or write all data structures to file. "
        "If reading, this will skip initialization phase. (read, write)\n");
    printf(
        "  -k <kernel ID>           Specifies which kernel to run. 0 is "
        "baseline, 1, 2, etc are optimized variants. (0 is default.)\n");
    printf(
        "Default is equivalent to: -m history -s large -l 34 -p 500000 -G "
        "unionized -k 0\n");
...
void binary_write(Inputs in, SimulationData SD) {
    ...
    // Write SimulationData Object. Include pointers, even though we won't be
    // using them.
    ...
    fwrite(SD.nuclide_grid, sizeof(NuclideGridPoint), SD.length_nuclide_grid,
           fp);
    fwrite(SD.index_grid, sizeof(int), SD.length_index_grid, fp);
    fwrite(SD.unionized_energy_array, sizeof(double),
           SD.length_unionized_energy_array, fp);
...
SimulationData binary_read(Inputs in) {
    ...
    // Read SimulationData Object. Include pointers, even though we won't be
    // using them.
    ...
    SD.nuclide_grid = (NuclideGridPoint*)malloc(SD.length_nuclide_grid *
                                                sizeof(NuclideGridPoint));
    SD.index_grid = (int*)malloc(SD.length_index_grid * sizeof(int));
    SD.unionized_energy_array =
        (double*)malloc(SD.length_unionized_energy_array * sizeof(double));
    ...
    fread(SD.nuclide_grid, sizeof(NuclideGridPoint), SD.length_nuclide_grid,
          fp);
    fread(SD.index_grid, sizeof(int), SD.length_index_grid, fp);
    fread(SD.unionized_energy_array, sizeof(double),
          SD.length_unionized_energy_array, fp);
