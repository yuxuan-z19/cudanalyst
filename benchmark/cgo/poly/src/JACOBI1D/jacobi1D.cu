...
void runJacobi1DCpu(int tsteps, int n, DATA_TYPE POLYBENCH_1D(A, N, n),
                    DATA_TYPE POLYBENCH_1D(B, N, n)) {
    for (int t = 0; t < _PB_TSTEPS; t++) {
        for (int i = 1; i < _PB_N - 1; i++)
            B[i] = 0.33333 * (A[i - 1] + A[i] + A[i + 1]);
        for (int j = 1; j < _PB_N - 1; j++) A[j] = B[j];
    }
}
...
void compareResults(int n, DATA_TYPE POLYBENCH_1D(a, N, n),
    ...
    // Compare a and c
    for (int i = 0; i < n; i++)
        fail += percentDiff(a[i], a_outputFromGpu[i]) >
                PERCENT_DIFF_ERROR_THRESHOLD;
    for (int i = 0; i < n; i++)
        fail += percentDiff(b[i], b_outputFromGpu[i]) >
                PERCENT_DIFF_ERROR_THRESHOLD;
...
void runJacobi1DCUDA(int tsteps, int n, DATA_TYPE POLYBENCH_1D(A, N, n),
                     DATA_TYPE POLYBENCH_1D(B, N, n),
                     DATA_TYPE POLYBENCH_1D(A_outputFromGpu, N, n),
                     DATA_TYPE POLYBENCH_1D(B_outputFromGpu, N, n)) {
    DATA_TYPE* Agpu;
    ...
    cudaMemcpy(B_outputFromGpu, Bgpu, sizeof(DATA_TYPE) * N,
               cudaMemcpyDeviceToHost);
...
int main(int argc, char** argv) {
    ...
    runJacobi1DCUDA(tsteps, n, POLYBENCH_ARRAY(a), POLYBENCH_ARRAY(b),
