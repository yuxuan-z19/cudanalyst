...
void gemver(int n, DATA_TYPE alpha, DATA_TYPE beta,
...
void init(int n, DATA_TYPE* alpha, DATA_TYPE* beta,
          DATA_TYPE POLYBENCH_2D(A, N, N, n, n),
          DATA_TYPE POLYBENCH_1D(u1, N, n), DATA_TYPE POLYBENCH_1D(v1, N, n),
          DATA_TYPE POLYBENCH_1D(u2, N, n), DATA_TYPE POLYBENCH_1D(v2, N, n),
          DATA_TYPE POLYBENCH_1D(w, N, n), DATA_TYPE POLYBENCH_1D(x, N, n),
    ...
    *beta = 12313;
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int j = 0; j < N; j++) A[i][j] = ((DATA_TYPE)i * j) / N;
    }
}

void compareResults(int n, DATA_TYPE POLYBENCH_1D(w1, N, n),
                    DATA_TYPE POLYBENCH_1D(w2, N, n)) {
    int fail = 0;
    for (int i = 0; i < N; i++)
        fail += percentDiff(w1[i], w2[i]) > PERCENT_DIFF_ERROR_THRESHOLD;
...
void gemverCuda(
    ...
    dim3 grid1((size_t)(ceil((float)N) / ((float)DIM_THREAD_BLOCK_KERNEL_1_X)),
               (size_t)(ceil((float)N) / ((float)DIM_THREAD_BLOCK_KERNEL_1_Y)));

    dim3 block2(DIM_THREAD_BLOCK_KERNEL_2_X, DIM_THREAD_BLOCK_KERNEL_2_Y);
    dim3 grid2((size_t)(ceil((float)N) / ((float)DIM_THREAD_BLOCK_KERNEL_2_X)),
               1);

    dim3 block3(DIM_THREAD_BLOCK_KERNEL_3_X, DIM_THREAD_BLOCK_KERNEL_3_Y);
    dim3 grid3((size_t)(ceil((float)N) / ((float)DIM_THREAD_BLOCK_KERNEL_3_X)),
               1);

    /* Start timer. */
    polybench_start_instruments;

    gemver_kernel1<<<grid1, block1>>>(n, alpha, beta, A_gpu, v1_gpu, v2_gpu,
                                      u1_gpu, u2_gpu);
    cudaDeviceSynchronize();
    gemver_kernel2<<<grid2, block2>>>(n, alpha, beta, A_gpu, x_gpu, y_gpu,
                                      z_gpu);
    cudaDeviceSynchronize();
    ...
    cudaMemcpy(w_outputFromGpu, w_gpu, sizeof(DATA_TYPE) * N,
...
int main(int argc, char* argv[]) {
    ...
    init(n, &alpha, &beta, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(u1),
    ...
    gemverCuda(n, alpha, beta, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(u1),
               POLYBENCH_ARRAY(v1), POLYBENCH_ARRAY(u2), POLYBENCH_ARRAY(v2),
               POLYBENCH_ARRAY(w), POLYBENCH_ARRAY(w_outputFromGpu),
               POLYBENCH_ARRAY(x), POLYBENCH_ARRAY(y), POLYBENCH_ARRAY(z));
    ...
    gemver(n, alpha, beta, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(u1),
           POLYBENCH_ARRAY(v1), POLYBENCH_ARRAY(u2), POLYBENCH_ARRAY(v2),
           POLYBENCH_ARRAY(w), POLYBENCH_ARRAY(x), POLYBENCH_ARRAY(y),
           POLYBENCH_ARRAY(z));
