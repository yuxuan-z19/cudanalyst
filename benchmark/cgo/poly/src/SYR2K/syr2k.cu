...
void syr2kCpu(int ni, int nj, DATA_TYPE alpha, DATA_TYPE beta,
    ...
    for (int i = 0; i < _PB_NI; i++)
        for (int j = 0; j < _PB_NI; j++) C[i][j] *= beta;
...
void syr2kCuda(int ni, int nj, DATA_TYPE alpha, DATA_TYPE beta,
               DATA_TYPE POLYBENCH_2D(A, NI, NJ, ni, nj),
               DATA_TYPE POLYBENCH_2D(B, NI, NJ, ni, nj),
               DATA_TYPE POLYBENCH_2D(C, NI, NI, ni, ni),
               DATA_TYPE POLYBENCH_2D(C_outputFromGpu, NI, NI, ni, ni)) {
...
int main() {
    ...
    syr2kCuda(ni, nj, alpha, beta, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(B),
    ...
    syr2kCpu(ni, nj, alpha, beta, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(B),
             POLYBENCH_ARRAY(C));
