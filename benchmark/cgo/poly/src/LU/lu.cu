...
void lu(int n, DATA_TYPE POLYBENCH_2D(A, N, N, n, n)) {
    for (int k = 0; k < _PB_N; k++) {
        for (int j = k + 1; j < _PB_N; j++) A[k][j] = A[k][j] / A[k][k];
...
void init_array(int n, DATA_TYPE POLYBENCH_2D(A, N, N, n, n)) {
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++) A[i][j] = ((DATA_TYPE)i * j + 1) / N;
}
...
void compareResults(int n, DATA_TYPE POLYBENCH_2D(A_cpu, N, N, n, n),
    ...
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++)
            fail += percentDiff(A_cpu[i][j], A_outputFromGpu[i][j]) >
...
void luCuda(int n, DATA_TYPE POLYBENCH_2D(A, N, N, n, n),
            DATA_TYPE POLYBENCH_2D(A_outputFromGpu, N, N, n, n)) {
    DATA_TYPE* AGpu;
    ...
    for (int k = 0; k < N; k++) {
        grid1.x =
            (unsigned int)(ceil((float)(N - (k + 1)) / ((float)block1.x)));
        lu_kernel1<<<grid1, block1>>>(n, AGpu, k);
        cudaDeviceSynchronize();

        grid2.x =
            (unsigned int)(ceil((float)(N - (k + 1)) / ((float)block2.x)));
        grid2.y =
            (unsigned int)(ceil((float)(N - (k + 1)) / ((float)block2.y)));
