...
void init_array(int nx, int ny, DATA_TYPE POLYBENCH_2D(A, NX, NY, nx, ny),
                DATA_TYPE POLYBENCH_1D(p, NY, ny),
                DATA_TYPE POLYBENCH_1D(r, NX, nx)) {
    int i, j;

    for (i = 0; i < ny; i++) p[i] = i * M_PI;

    for (i = 0; i < nx; i++) {
        r[i] = i * M_PI;
        for (j = 0; j < ny; j++) A[i][j] = ((DATA_TYPE)i * j) / NX;
    }
}

void compareResults(int nx, int ny, DATA_TYPE POLYBENCH_1D(s, NY, ny),
    ...
    for (i = 0; i < nx; i++)
        fail += percentDiff(q[i], q_outputFromGpu[i]) >
                PERCENT_DIFF_ERROR_THRESHOLD;

    for (i = 0; i < ny; i++)
        fail += percentDiff(s[i], s_outputFromGpu[i]) >
                PERCENT_DIFF_ERROR_THRESHOLD;
...
void bicg_cpu(int nx, int ny, DATA_TYPE POLYBENCH_2D(A, NX, NY, nx, ny),
              DATA_TYPE POLYBENCH_1D(r, NX, nx),
...
void bicgCuda(int nx, int ny, DATA_TYPE POLYBENCH_2D(A, NX, NY, nx, ny),
              DATA_TYPE POLYBENCH_1D(r, NX, nx),
              DATA_TYPE POLYBENCH_1D(s, NY, ny),
              DATA_TYPE POLYBENCH_1D(p, NY, ny),
              DATA_TYPE POLYBENCH_1D(q, NX, nx),
...
int main(int argc, char** argv) {
    ...
    init_array(nx, ny, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(p),
               POLYBENCH_ARRAY(r));

    GPU_argv_init();

    bicgCuda(nx, ny, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(r), POLYBENCH_ARRAY(s),
    ...
    bicg_cpu(nx, ny, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(r), POLYBENCH_ARRAY(s),
             POLYBENCH_ARRAY(p), POLYBENCH_ARRAY(q));
    ...
    compareResults(nx, ny, POLYBENCH_ARRAY(s), POLYBENCH_ARRAY(s_outputFromGpu),
                   POLYBENCH_ARRAY(q), POLYBENCH_ARRAY(q_outputFromGpu));

#else  // print output to stderr so no dead code elimination

    print_array(nx, ny, POLYBENCH_ARRAY(s_outputFromGpu),
                POLYBENCH_ARRAY(q_outputFromGpu));
