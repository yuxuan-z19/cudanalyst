...
void init_array(int ni, int nj, int nk, int nl, int nm,
    ...
    for (i = 0; i < ni; i++)
        for (j = 0; j < nk; j++) A[i][j] = ((DATA_TYPE)i * j) / ni;

    for (i = 0; i < nk; i++)
        for (j = 0; j < nj; j++) B[i][j] = ((DATA_TYPE)i * (j + 1)) / nj;

    for (i = 0; i < nj; i++)
        for (j = 0; j < nm; j++) C[i][j] = ((DATA_TYPE)i * (j + 3)) / nl;

    for (i = 0; i < nm; i++)
        for (j = 0; j < nl; j++) D[i][j] = ((DATA_TYPE)i * (j + 2)) / nk;
}

void compareResults(int ni, int nl, DATA_TYPE POLYBENCH_2D(G, NI, NL, ni, nl),
                    DATA_TYPE POLYBENCH_2D(G_outputFromGpu, NI, NL, ni, nl)) {
    int fail = 0;

    for (int i = 0; i < ni; i++)
        for (int j = 0; j < nl; j++)
            fail += percentDiff(G[i][j], G_outputFromGpu[i][j]) >
...
void mm3_cpu(int ni, int nj, int nk, int nl, int nm,
    ...
    for (i = 0; i < _PB_NJ; i++)
        for (j = 0; j < _PB_NL; j++) {
            F[i][j] = 0;
            for (k = 0; k < _PB_NM; ++k) F[i][j] += C[i][k] * D[k][j];
        }
...
int main(int argc, char** argv) {
    ...
    mm3Cuda(ni, nj, nk, nl, nm, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(B),
            POLYBENCH_ARRAY(C), POLYBENCH_ARRAY(D), POLYBENCH_ARRAY(E),
    ...
    mm3_cpu(ni, nj, nk, nl, nm, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(B),
            POLYBENCH_ARRAY(C), POLYBENCH_ARRAY(D), POLYBENCH_ARRAY(E),
            POLYBENCH_ARRAY(F), POLYBENCH_ARRAY(G));
    ...
    compareResults(ni, nl, POLYBENCH_ARRAY(G),
                   POLYBENCH_ARRAY(G_outputFromGpu));
