...
void init_array(int ni, int nj, int nk, int nl, DATA_TYPE* alpha,
    ...
    *beta = BETA;
    ...
    for (int i = 0; i < ni; i++)
        for (int j = 0; j < nk; j++) A[i][j] = ((DATA_TYPE)i * j) / NI;

    for (int i = 0; i < nk; i++)
        for (int j = 0; j < nj; j++) B[i][j] = ((DATA_TYPE)i * (j + 1)) / NJ;

    for (int i = 0; i < nl; i++)
        for (int j = 0; j < nj; j++) C[i][j] = ((DATA_TYPE)i * (j + 3)) / NL;

    for (int i = 0; i < ni; i++)
        for (int j = 0; j < nl; j++) D[i][j] = ((DATA_TYPE)i * (j + 2)) / NK;
}
...
void compareResults(int ni, int nl, DATA_TYPE POLYBENCH_2D(D, NI, NL, ni, nl),
    ...
    for (int i = 0; i < ni; i++)
        for (int j = 0; j < nl; j++)
            fail += percentDiff(D[i][j], D_outputFromGpu[i][j]) >
...
void mm2_cpu(int ni, int nj, int nk, int nl, DATA_TYPE alpha, DATA_TYPE beta,
    ...
    for (i = 0; i < _PB_NI; i++)
        for (j = 0; j < _PB_NL; j++) {
            D[i][j] *= beta;
            for (k = 0; k < _PB_NJ; ++k) D[i][j] += tmp[i][k] * C[k][j];
        }
}
...
void mm2Cuda(int ni, int nj, int nk, int nl, DATA_TYPE alpha, DATA_TYPE beta,
    ...
    cudaMemcpy(tmp_gpu, tmp, sizeof(DATA_TYPE) * NI * NJ,
...
int main(int argc, char** argv) {
    ...
    mm2Cuda(ni, nj, nk, nl, alpha, beta, POLYBENCH_ARRAY(tmp),
    ...
    mm2_cpu(ni, nj, nk, nl, alpha, beta, POLYBENCH_ARRAY(tmp),
            POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(B), POLYBENCH_ARRAY(C),
            POLYBENCH_ARRAY(D));
    ...
    compareResults(ni, nl, POLYBENCH_ARRAY(D),
