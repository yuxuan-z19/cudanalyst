...
void adi(int tsteps, int n, DATA_TYPE POLYBENCH_2D(A, N, N, n, n),
...
void compareResults(int n, DATA_TYPE POLYBENCH_2D(B_cpu, N, N, n, n),
                    DATA_TYPE POLYBENCH_2D(B_fromGpu, N, N, n, n),
                    DATA_TYPE POLYBENCH_2D(X_cpu, N, N, n, n),
    ...
    for (i = 0; i < n; i++)
        for (j = 0; j < n; j++)
            fail += percentDiff(B_cpu[i][j], B_fromGpu[i][j]) >
                    PERCENT_DIFF_ERROR_THRESHOLD;

    for (i = 0; i < n; i++)
        for (j = 0; j < n; j++)
            fail += percentDiff(X_cpu[i][j], X_fromGpu[i][j]) >
                    PERCENT_DIFF_ERROR_THRESHOLD;
...
void adiCuda(int tsteps, int n, DATA_TYPE POLYBENCH_2D(A, N, N, n, n),
             DATA_TYPE POLYBENCH_2D(B, N, N, n, n),
             DATA_TYPE POLYBENCH_2D(X, N, N, n, n),
    ...
    cudaMemcpy(B_outputFromGpu, B_gpu, N * N * sizeof(DATA_TYPE),
               cudaMemcpyDeviceToHost);
    cudaMemcpy(X_outputFromGpu, X_gpu, N * N * sizeof(DATA_TYPE),
               cudaMemcpyDeviceToHost);
...
int main(int argc, char* argv[]) {
    ...
    adiCuda(tsteps, n, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(B),
            POLYBENCH_ARRAY(X), POLYBENCH_ARRAY(B_outputFromGpu),
    ...
    compareResults(n, POLYBENCH_ARRAY(B), POLYBENCH_ARRAY(B_outputFromGpu),
                   POLYBENCH_ARRAY(X), POLYBENCH_ARRAY(X_outputFromGpu));
