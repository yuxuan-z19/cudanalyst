...
void gramschmidt(int ni, int nj, DATA_TYPE POLYBENCH_2D(A, NI, NJ, ni, nj),
    ...
    for (int k = 0; k < _PB_NJ; k++) {
        nrm = 0;
        for (i = 0; i < _PB_NI; i++) nrm += A[i][k] * A[i][k];

        R[k][k] = sqrt(nrm);
        for (i = 0; i < _PB_NI; i++) Q[i][k] = A[i][k] / R[k][k];

        for (j = k + 1; j < _PB_NJ; j++) {
            R[k][j] = 0;
            for (i = 0; i < _PB_NI; i++) R[k][j] += Q[i][k] * A[i][j];
            for (i = 0; i < _PB_NI; i++) A[i][j] = A[i][j] - Q[i][k] * R[k][j];
        }
    }
}

/* Array initialization. */
void init_array(int ni, int nj, DATA_TYPE POLYBENCH_2D(A, NI, NJ, ni, nj),
                DATA_TYPE POLYBENCH_2D(R, NJ, NJ, nj, nj),
                DATA_TYPE POLYBENCH_2D(Q, NI, NJ, ni, nj)) {
    int i, j;
    for (i = 0; i < ni; i++)
    ...
    for (i = 0; i < nj; i++)
        for (j = 0; j < nj; j++) R[i][j] = ((DATA_TYPE)i * (j + 2)) / nj;
}
...
void compareResults(int ni, int nj, DATA_TYPE POLYBENCH_2D(A, NI, NJ, ni, nj),
    ...
    for (int i = 0; i < ni; i++)
        for (int j = 0; j < nj; j++)
            fail += percentDiff(A[i][j], A_outputFromGpu[i][j]) >
...
void gramschmidtCuda(int ni, int nj, DATA_TYPE POLYBENCH_2D(A, NI, NJ, ni, nj),
                     DATA_TYPE POLYBENCH_2D(R, NJ, NJ, nj, nj),
                     DATA_TYPE POLYBENCH_2D(Q, NI, NJ, ni, nj),
                     DATA_TYPE POLYBENCH_2D(A_outputFromGpu, NI, NJ, ni, nj)) {
    dim3 block(DIM_THREAD_BLOCK_X, DIM_THREAD_BLOCK_Y);
    dim3 gridKernel1(1, 1);
    dim3 gridKernel2((size_t)ceil(((float)NJ) / ((float)DIM_THREAD_BLOCK_X)),
                     1);
    dim3 gridKernel3((size_t)ceil(((float)NJ) / ((float)DIM_THREAD_BLOCK_X)),
                     1);
    ...
    for (int k = 0; k < _PB_NJ; k++) {
        ...
        gramschmidt_kernel2<<<gridKernel2, block>>>(ni, nj, A_gpu, R_gpu, Q_gpu,
                                                    k);
        cudaDeviceSynchronize();
        gramschmidt_kernel3<<<gridKernel3, block>>>(ni, nj, A_gpu, R_gpu, Q_gpu,
                                                    k);
        cudaDeviceSynchronize();
    }
    printf("GPU_Seconds=");
...
int main(int argc, char* argv[]) {
    ...
    init_array(ni, nj, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(R),
               POLYBENCH_ARRAY(Q));

    GPU_argv_init();

    gramschmidtCuda(ni, nj, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(R),
                    POLYBENCH_ARRAY(Q), POLYBENCH_ARRAY(A_outputFromGpu));
    ...
    gramschmidt(ni, nj, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(R),
                POLYBENCH_ARRAY(Q));
    ...
    compareResults(ni, nj, POLYBENCH_ARRAY(A),
