...
void covariance(int m, int n, DATA_TYPE POLYBENCH_2D(data, M, N, m, n),
...
void compareResults(int m, int n, DATA_TYPE POLYBENCH_2D(symmat, M, M, m, m),
...
void covarianceCuda(int m, int n, DATA_TYPE POLYBENCH_2D(data, M, N, m, n),
                    DATA_TYPE POLYBENCH_2D(symmat, M, M, m, m),
                    DATA_TYPE POLYBENCH_1D(mean, M, m),
                    DATA_TYPE POLYBENCH_2D(symmat_outputFromGpu, M, M, m, m)) {
    DATA_TYPE* data_gpu;
    ...
    cudaMemcpy(symmat_gpu, symmat, sizeof(DATA_TYPE) * M * M,
               cudaMemcpyHostToDevice);
...
int main() {
    ...
    covarianceCuda(m, n, POLYBENCH_ARRAY(data), POLYBENCH_ARRAY(symmat),
    ...
    covariance(m, n, POLYBENCH_ARRAY(data), POLYBENCH_ARRAY(symmat),
               POLYBENCH_ARRAY(mean));
