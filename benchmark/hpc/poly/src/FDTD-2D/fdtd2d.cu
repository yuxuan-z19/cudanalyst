...
void init_arrays(int tmax, int nx, int ny,
...
void runFdtd(int tmax, int nx, int ny,
             DATA_TYPE POLYBENCH_1D(_fict_, TMAX, TMAX),
             DATA_TYPE POLYBENCH_2D(ex, NX, NY, nx, ny),
             DATA_TYPE POLYBENCH_2D(ey, NX, NY, nx, ny),
             DATA_TYPE POLYBENCH_2D(hz, NX, NY, nx, ny)) {
    ...
    for (int t = 0; t < _PB_TMAX; t++) {
        for (j = 0; j < _PB_NY; j++) ey[0][j] = _fict_[t];
...
void compareResults(int nx, int ny, DATA_TYPE POLYBENCH_2D(hz1, NX, NY, nx, ny),
                    DATA_TYPE POLYBENCH_2D(hz2, NX, NY, nx, ny)) {
    int fail = 0;
    for (int i = 0; i < nx; i++)
        for (int j = 0; j < ny; j++)
            fail += percentDiff(hz1[i][j], hz2[i][j]) >
...
void fdtdCuda(int tmax, int nx, int ny,
              DATA_TYPE POLYBENCH_1D(_fict_, TMAX, TMAX),
              DATA_TYPE POLYBENCH_2D(ex, NX, NY, nx, ny),
              DATA_TYPE POLYBENCH_2D(ey, NX, NY, nx, ny),
              DATA_TYPE POLYBENCH_2D(hz, NX, NY, nx, ny),
    ...
    for (int t = 0; t < _PB_TMAX; t++) {
        fdtd_step1_kernel<<<grid, block>>>(nx, ny, _fict_gpu, ex_gpu, ey_gpu,
                                           hz_gpu, t);
...
int main() {
    ...
    init_arrays(tmax, nx, ny, POLYBENCH_ARRAY(_fict_), POLYBENCH_ARRAY(ex),
                POLYBENCH_ARRAY(ey), POLYBENCH_ARRAY(hz));

    GPU_argv_init();
    fdtdCuda(tmax, nx, ny, POLYBENCH_ARRAY(_fict_), POLYBENCH_ARRAY(ex),
             POLYBENCH_ARRAY(ey), POLYBENCH_ARRAY(hz),
    ...
    runFdtd(tmax, nx, ny, POLYBENCH_ARRAY(_fict_), POLYBENCH_ARRAY(ex),
            POLYBENCH_ARRAY(ey), POLYBENCH_ARRAY(hz));
    ...
    compareResults(nx, ny, POLYBENCH_ARRAY(hz),
                   POLYBENCH_ARRAY(hz_outputFromGpu));
